                    ...
                    <form method="POST" action="{{ route('tenant.projects.update', $project) }}">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <x-input id="title" class="block w-full mt-1" type="text" name="title" :value="$project->title" autofocus autocomplete="title" />
                                @error('title')
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="6" class="block w-full mt-1">{{ $project->description }}</textarea>
                                @error('description')
                                    <div class="alert alert-danger mt-3 text-sm text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="deadline_at" class="rounded" value="{{ $project->deadline_at }}"/>
                                @error('deadline_at')
                                    <div class="alert alert-danger mt-3 text-sm text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select name="tenant_id" id="tenant_id" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach ($tenants as $tenant)
                                        <option value="{{ $tenant->id }}"
                                            @if($project->tenant->id == $tenant->id)
                                                selected
                            ...
                            <div>
                                ...
                                <select name="status" id="status" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach (App\Enums\Status::cases() as $status)
                                        <option
                                            value="{{ $status->value }}" {{ old('status', $project->status->value) != $status->value ?: 'selected' }}>
                                            {{ $status->text() }}
                                        </option>
                                    @endforeach
                                </select>
